  ...
  const stepper = (side: 'A' | 'B', value: number, label: string) => (
    <div className="flex-1 text-center">
      ...
      <div className="flex items-center justify-center gap-3">
        <button onClick={() => send({ action: 'bump', id: side, delta: -step })} className="w-16 h-16 rounded-full bg-slate-200 active:bg-slate-300 grid place-items-center" aria-label={`Decrease ${label}`}>
          <Minus className="w-7 h-7" />
        </button>
        <button onClick={() => send({ action: 'bump', id: side, delta: step })} className="w-16 h-16 rounded-full bg-primary text-primary-foreground active:brightness-95 grid place-items-center" aria-label={`Increase ${label}`}>
