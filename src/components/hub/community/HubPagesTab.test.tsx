...
it('renders an approved page with a link to it', () => {
  render(<HubPagesTab hubId="h1" canManage={false} canAttach currentUserId="u1" initialPages={[approved]} />)
...
it('shows the review queue to a moderator', () => {
  render(<HubPagesTab hubId="h1" canManage canAttach currentUserId="mod" initialPages={[approved, pending]} />)
...
it('badges the attacher own pending row', () => {
  render(<HubPagesTab hubId="h1" canManage={false} canAttach currentUserId="u2" initialPages={[approved, pending]} />)
...
it('shows a moderator their own rejected row without duplicating their own pending row from the queue', () => {
  ...
  render(<HubPagesTab hubId="h1" canManage canAttach currentUserId="mod" initialPages={[approved, pending, modPending, modRejected]} />)
...
it('shows an empty state when nothing is attached', () => {
  render(<HubPagesTab hubId="h1" canManage={false} canAttach currentUserId="u1" initialPages={[]} />)
...
it('reflects a newly attached page when initialPages changes after a router.refresh()', () => {
  const { rerender } = render(
    <HubPagesTab hubId="h1" canManage={false} canAttach currentUserId="u1" initialPages={[]} />,
  )
  expect(screen.getByText(/no pages yet/i)).toBeInTheDocument()
  ...
  rerender(
    <HubPagesTab hubId="h1" canManage={false} canAttach currentUserId="u1" initialPages={[attached]} />,
  )
...
it('hides the Attach a Page trigger from a signed-in non-member', () => {
  render(<HubPagesTab hubId="h1" canManage={false} canAttach={false} currentUserId="u3" initialPages={[approved]} />)
  expect(screen.queryByRole('button', { name: /attach a page/i })).not.toBeInTheDocument()
})

it('shows the Attach a Page trigger to a privileged user even when canAttach is false', () => {
  render(<HubPagesTab hubId="h1" canManage canAttach={false} currentUserId="mod" initialPages={[approved]} />)
  expect(screen.getByRole('button', { name: /attach a page/i })).toBeInTheDocument()
})

it('renders no empty-state shell for a signed-in non-member when nothing is attached', () => {
  const { container } = render(
    <HubPagesTab hubId="h1" canManage={false} canAttach={false} currentUserId="u3" initialPages={[]} />,
  )
  expect(screen.queryByText(/no pages yet/i)).not.toBeInTheDocument()
  expect(screen.queryByRole('button', { name: /attach a page/i })).not.toBeInTheDocument()
...
it('renders no empty-state shell for a logged-out viewer when nothing is attached', () => {
  render(<HubPagesTab hubId="h1" canManage={false} canAttach={false} currentUserId={null} initialPages={[]} />)
  expect(screen.queryByText(/no pages yet/i)).not.toBeInTheDocument()
})
