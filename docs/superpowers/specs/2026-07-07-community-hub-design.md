...
7. A lightweight **`/communities`** dashboard list of the hubs the viewer has joined, each with its latest post — so members can get back easily.
...
## Pro-gating (recommended default — user may veto)

- **Enabling the community layer** on a hub → **Pro** (creator/audience feature; consistent with Hub privacy + Boards being Pro). Non-Pro owner sees the in-app **Pro** prompt (the `UpgradePrompt` modal — no enterprise redirect).
...
- **Member feed** `src/app/(dashboard)/communities/page.tsx` — lists hubs the viewer has joined + each hub's latest post; links to the hub viewer.
