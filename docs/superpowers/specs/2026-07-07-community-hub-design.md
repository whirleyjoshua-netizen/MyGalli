...
3. `HubPost` broadcast feed — **text + optional image + likes** for v1. Authored by owner or collaborators. Likes mirror `BulletinLike`. The `blocks`/`settings` JSON columns and a `HubPostResponse` table are added to the schema now (cheap, future-proof) but **interactive poll/rating blocks are deferred to Phase 2** — wiring them would require threading a new endpoint prop through the shared `BulletinBlock` response stack (poll/rating/shortanswer children are hardcoded to `/api/bulletin`), which is disproportionate coupling for v1. Rendering reuses `BulletinPostCard` (with an added `basePath` prop) whose block slot is simply null in v1.
...
- **Interactive poll/rating/short-answer blocks on posts** — schema is ready (`blocks`/`settings`/`HubPostResponse`); wiring reuses the Bulletin block stack once it's parametrized by a `basePath`.
...
- `POST /api/hubs/[id]/posts/[postId]/like` — members (signed-in), toggles like. (`.../respond` deferred to Phase 2 with interactive blocks.)
...
- **Owner console** `src/app/(dashboard)/hubs/[id]` — a **Posts** tab (a small dedicated `HubPostComposer`: text + image, no block editor in v1) + a **Members** tab (list, remove). Collaborators see the composer too.
